...
public interface LlmUsageRepository extends JpaRepository<LlmUsage, UUID> {
    ...
    @Query(value = "SELECT CAST(NULL AS VARCHAR) AS groupKey, " + AGGREGATE_COLUMNS
            + "FROM llm_usage WHERE created_at >= :from AND created_at <= :to",
            nativeQuery = true)
    ...
    @Query(value = "SELECT provider AS groupKey, " + AGGREGATE_COLUMNS
            + "FROM llm_usage WHERE created_at >= :from AND created_at <= :to GROUP BY provider ORDER BY provider",
            nativeQuery = true)
    ...
    @Query(value = "SELECT model AS groupKey, " + AGGREGATE_COLUMNS
            + "FROM llm_usage WHERE created_at >= :from AND created_at <= :to GROUP BY model ORDER BY model",
            nativeQuery = true)
    ...
    @Query(value = "SELECT feature AS groupKey, " + AGGREGATE_COLUMNS
            + "FROM llm_usage WHERE created_at >= :from AND created_at <= :to GROUP BY feature ORDER BY feature",
            nativeQuery = true)
